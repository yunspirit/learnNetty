...
public class UDFTest {
  ...
  public void simpleInput() throws Exception{
    BaseRunner runner = new UDFRunner(null, "udf.CompareStr");
    runner.feed(new Object[] { "one", "one" }).feed(new Object[] { "three,four", "three" })
        .feed(new Object[] { "four,five", "four" });
    List<Object[]> out = runner.yield();

    Assert.assertEquals(3, out.size());
    Assert.assertEquals("true", TestUtil.join(out.get(0)));
    Assert.assertEquals("false", TestUtil.join(out.get(1)));
    Assert.assertEquals("false", TestUtil.join(out.get(2)));
  }
